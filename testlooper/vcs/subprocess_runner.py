...
class SubprocessRunner(object):
    ...
    def start(self):
        ...
        self.subprocessOutThread = ManagedThread(
        ...
        self.subprocessErrThread = ManagedThread(
            target=self.processOutputLoop,
        ...
        startSubprocessThread = ManagedThread(target=startSubprocess)
    ...
    def write(self, content):
        ...
        assert self.subprocessStdIn is not None, "Process stdin is not open."
    ...
    def stop(self):
        try:
            ...
            if self.subprocessOutThread is not None and not self.isSuprocessOutThread():
                assert self.subprocessStdOutFromOtherSide is not None
            ...
            if self.subprocessErrThread is not None and not self.isSuprocessErrThread():
                assert self.subprocessStdErrFromOtherSide is not None
    ...
    def isSuprocessOutThread(self):
        assert self.subprocessOutThread is not None
...
def callAndReturnResultWithoutOutput(args, timeout=60.0, shell=False, env=None):
    sub_process = SubprocessRunner(args, lambda _: None, lambda _: None, shell=shell, env=env)
    sub_process.start()
