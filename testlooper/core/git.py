...
class SubprocessCheckCall(object):
    ...
    def __call__(self):
        with from_directory(self.path):
            try:
                subprocess.check_output(*self.args, stderr=subprocess.STDOUT, **self.kwds)
...
class Git:
    ...
    def pullLatest(self, remote="origin"):
        remotes = self.listRemotes()
        if remote in remotes:
            return self.subprocessCheckCall(["git", "fetch", remote, "--prune"]) == 0
        else:
            return True
    ...
    def createInitialCommit(self, commitMessage="Initial commit"):
        with self.git_repo_lock:
            ...
            try:
                assert (
                    self.subprocessCheckCall(["git", "checkout", "--orphan", branchname]) == 0
                )
    ...
    def listBranches(self):
        with self.git_repo_lock:
            output = (
                self.subprocessCheckOutput(["git", "branch", "--list"]).strip().split("\n")
            )
            ...
            return [line for line in output if line and self.isValidBranchName_(line)]
    ...
    def distanceForCommitInBranch(self, hash, branch, maxSearchDepth=100):
    ...
    def listCurrentlyKnownBranchesForRemote(self, remote):
        ...
        with self.git_repo_lock:
            ...
            lines = [line[7:].strip() for line in lines if line.startswith("origin/")]
    ...
    def listBranchesForRemote(self, remote):
        ...
        with self.git_repo_lock:
            lines = (
                self.subprocessCheckOutput(["git", "ls-remote", remote]).strip().split("\n")
            )
    ...
    def getTestDefinitionsPathFromDir(checkoutPath):
        all = []
        with from_directory(checkoutPath):
    ...
    def fetchOrigin(self):
        with self.git_repo_lock:
            if self.subprocessCheckCall(["git", "fetch", "--prune"]) != 0:
    ...
    def createRepoTarball(self, commitHash, pathWithinRepo, targetTarball, setCoreAutocrlf):
        """Create a tarball for subset of a commit

        Checkout and tarball the subpath 'pathWithinRepo' from commit commitHash
        and place at targetTarball.
        """
